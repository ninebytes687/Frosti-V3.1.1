...
**Example：**  
test1-servers组内所有主机创建ninebytes_admin用户组：  
```shell
[root@server ~]# ansible test1-servers -i /etc/ansible/inventory2.ini -m group -a "name=ninebytes_admin"
192.168.201.12 | CHANGED => {
    "ansible_facts": {
        "discovered_interpreter_python": "/usr/bin/python3"
    },
...
```
test1-servers组内所有主机删除ninebytes_admin用户组：  
```shell
[root@server ~]# ansible test1-servers -i /etc/ansible/inventory2.ini -m group -a "state=absent name=ninebytes_admin"
192.168.201.12 | CHANGED => {
    "ansible_facts": {
        "discovered_interpreter_python": "/usr/bin/python3"
    },
...
- remove：是否删除与用户关联的目录，只有当state=absent时生效。删除设置为yes，默认no 不删除；  
...
**Example：**  
**【1】**：test1-servers组内所有主机创建用户testuser，设置为系统用户，登录密码为123456，加入testgroup1用户组。  
...
```shell
[root@server ~]# ansible test1-servers -i /etc/ansible/inventory2.ini -m user -a "name=testuser system=yes password=$PASS_HASH group=testgroup1"
192.168.201.12 | CHANGED => {
    "ansible_facts": {
        "discovered_interpreter_python": "/usr/bin/python3"
    },
...
```shell
[root@server ~]# ansible test1-servers -i /etc/ansible/inventory2.ini -m shell -a "grep testuser /etc/shadow"
...
**【2】**：test1-servers组内所有主机删除testuser用户。  
```shell
[root@server ~]# ansible test1-servers -i /etc/ansible/inventory2.ini -m user -a "name=testuser state=absent"
[WARNING]: Invalid characters were found in group names but not replaced, use -vvvv to see details
192.168.201.12 | CHANGED => {
    "ansible_facts": {
        "discovered_interpreter_python": "/usr/bin/python3"
...
**【3】**：test1-servers组内所有主机创建用户testuser2，并为其创建秘钥对，秘钥类型为ecdsa。  
```shell
[root@server ~]# ansible test1-servers -i /etc/ansible/inventory2.ini -m user -a "name=testuser2 generate_ssh_key=yes ssh_key_type=ecdsa ssh_key_bits=256"
192.168.201.12 | CHANGED => {
    "ansible_facts": {
        "discovered_interpreter_python": "/usr/bin/python3"
    },
...
查看密钥对是否创建成功：  
```shell
[root@server ~]# ansible test1-servers -i /etc/ansible/inventory2.ini -m shell -a "ls /home/testuser2/.ssh"
192.168.201.12 | CHANGED | rc=0 >>
id_ecdsa
id_ecdsa.pub
```
